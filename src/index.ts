...
const planetsEl: NodeListOf<Element> =
  document.querySelectorAll(".planets_planet");
let solarSystem: IPlanet[] = [];

// using interface IPlanet for the planets in solarsystem.
interface IPlanet {
  id: number;
  name: string;
  latinName: string;
...
getApiKey();
const fetchSolaris = async (key: string) => {
...
//creates the overlay
const createOverlay = (planet: Element, i: number, event: Event): string => {
  const currentPlanet: IPlanet = solarSystem[i];
  const planetStyles: CSSStyleDeclaration = window.getComputedStyle(planet);
  const planetBgColor: string = planetStyles.backgroundColor;
  const borderColor1: string = planetBgColor
    .replace("rgb", "rgba")
    .replace(")", ", 0.1)");
  const borderColor2: string = planetBgColor
    .replace("rgb", "rgba")
    .replace(")", ", 0.06)");
  const overlay = `
...
const setOverlay = (overlay: string) => {
  ...
  const bigInfoPlanetEl: HTMLElement = document.getElementById("bigInfoPlanet");
...
planetsEl.forEach((planet, i) => {
  planet.addEventListener("click", (event) => {
    const overlay: string = createOverlay(planet, i, event);
    setOverlay(overlay);
  });
});
